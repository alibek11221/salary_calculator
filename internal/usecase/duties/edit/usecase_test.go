...
import (
	...
	edit_duty_uc "salary_calculator/internal/usecase/duties/edit"
...
func TestUsecase_Do(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(f fields) {
				f.r.EXPECT().UpdateDuty(gomock.Any(), dbstore.UpdateDutyParams{
					ID:          pgID,
					Value:       75000,
					Date:        sd.String(),
					Coefficient: 2.0,
		...
		},
		{
			...
			setup: func(f fields) {
				f.r.EXPECT().UpdateDuty(gomock.Any(), gomock.Any()).Return(errors.New("db error"))
			},
			wantErr: true,
		},
	}
