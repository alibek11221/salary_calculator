...
import (
	...
	"salary_calculator/internal/usecase/salary_change/add"
...
func TestUsecase_Do(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(f fields) {
				f.r.EXPECT().InsertChange(gomock.Any(), dbstore.InsertChangeParams{
					Salary:     100000,
		...
		},
		{
			...
			wantErr: add.ErrDuplicateSalaryChange,
		},
		{
			name: "other error",
			args: args{
			...
			setup: func(f fields) {
				f.r.EXPECT().InsertChange(gomock.Any(), gomock.Any()).Return(errors.New("db error"))
			},
